            ...
            <Box marginRight={5}>
              <PlatformSelector
                ...
                onSelectedPlatform={(platform) =>
                  setGameQuery({ ...gameQuery, platform, pageNumber: 1})
                }
              />
            </Box>
            <SortSelector
              selectedSortOrder={gameQuery.sortOrder.split(":")[0]}
              onSelectSortOrder={(sortOrder) =>
                setGameQuery({ ...gameQuery, sortOrder, pageNumber: 1})}
            />
          </Flex>
        </Box>
        <GameGrid onChange={(len) => setResultLen(len)}gameQuery={gameQuery} />
        <PageSelector 
            resultLen = {resultLen}
          selectedPage={gameQuery.pageNumber} 
          onSelectedPage={(pageNumber) => setGameQuery({...gameQuery, pageNumber})}/>
      </GridItem>
    </Grid>
